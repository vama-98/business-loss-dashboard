...
def calculate_business_loss(inventory_url, arr_drr_url, b2b_url, start_date, end_date):
    ...
    if not {"variant_id", "product_title", "drr", "asp", "sku"}.issubset(arr_drr.columns):
        st.warning("⚠️ ARR/DRR sheet missing required columns.")
        return report, tidy
...
if report is not None and not report.empty:
    ...
    def highlight_doh(row):
        if row["latest_inventory"] == 0:
            return ["background-color: #FFC7C7"] * len(row)
        elif 0 < row["doh"] <= 7:
            return ["background-color: #FFC7C7"] * len(row)
        elif 8 <= row["doh"] <= 15:
            return ["background-color: #fff6a5"] * len(row)
        else:
            return [""] * len(row)
    ...
    try:
        ...
        st.error(f"⚠️ Styling failed: {e}")
        st.dataframe(report.fillna(0), width="stretch")
