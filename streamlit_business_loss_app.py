...
def calculate_business_loss(inventory_url, arr_drr_url, b2b_url, start_date, end_date):
    ...
    report["variant_label"] = report.apply(
        lambda x: f"{x['product_title']} ({x['variant_id']})" if pd.notna(x["product_title"]) else x["variant_id"], axis=1
    )
...
if start_date and end_date and start_date > end_date:
    st.error("❌ Invalid date range selected. Start Date cannot be after End Date.")
    st.stop()
...
if report is not None and not report.empty:
    ...
    try:
        styled_df = (
            report[["variant_label", "latest_inventory", "b2b_inventory", "doh", "days_out_of_stock", "drr", "asp", "business_loss"]]
            .style.apply(highlight_doh, axis=1)
            .format({"latest_inventory": "{:.0f}", "b2b_inventory": "{:.0f}", "doh": "{:.0f}",
                     "drr": "{:.1f}", "asp": "₹{:.0f}", "business_loss": "₹{:.0f}"})
        )
        st.dataframe(styled_df, use_container_width=True)
    except Exception:
        st.dataframe(report[["variant_label", "latest_inventory", "b2b_inventory", "doh",
                             "days_out_of_stock", "drr", "asp", "business_loss"]], use_container_width=True)

    # -------------------------------
    # PIE CHART (>3% CONTRIBUTION)
    # -------------------------------
    ...
    if not pie_df.empty:
        fig2 = px.pie(pie_df, names="variant_label", values="business_loss",
                      title="Contribution to Total Business Loss (Active SKUs >3%)",
                      color_discrete_sequence=px.colors.sequential.RdBu)
        st.plotly_chart(fig2, use_container_width=True)

    # -------------------------------
    # D2C PERFORMANCE INSIGHTS
    # -------------------------------
    st.markdown("---")
