...
def calculate_business_loss(inventory_url, arr_drr_url, b2b_url, start_date, end_date):
    ...
    report["variant_label"] = report.apply(
        ...
        if pd.notna(x["product_title"]) and x["product_title"].strip() != ""
        else f"({x['variant_id']})",
        axis=1
    )
...
if report is not None and not report.empty:
    ...
    try:
        ...
        st.warning(f"⚠️ Styling failed ({e.__class__.__name__}) — showing plain table instead.")
    ...
    if st.sidebar.button("Simulate Impact"):
        ...
        if drr <= 0:
            ...
            if new_doh < 15:
                st.sidebar.warning(f"⚠️ Blocking will reduce DOH to {new_doh} days (low for D2C).")
            else:
